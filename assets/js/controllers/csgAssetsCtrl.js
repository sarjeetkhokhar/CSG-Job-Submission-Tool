...
csgJobSubTool.controller('csgAssetsCtrl', function($scope,$sce,docParams,$stateParams,csgServicesFactory,$window,$uibModal,$location,currentUser,csgTest) {
     ...
     fileArr.length = fileArr.length - 2
         ...
         for (let i = 0; i < fileArr.length; i++){
             ...
             reader.onloadend = function(e){
                 ...
                 var strAjaxUrl = "https://eygb.sharepoint.com/sites/CreativeWorkflow/_api/web/lists/getByTitle('Lst_DocumentLibrary')/RootFolder/files/Add(url='"+ fileName +"',overwrite=true)";
           ...
           $('#successDiv').append(`<ul><li><span>`+ arr[0].Name +`</span><br/>
           <span>`+ $scope.userName + `</span><br/>
           <span>date and time(`+ getFormatedDate() +`)</span><br/>
           <span class="delete" nameAttr=`+ arr[0].Name +` onclick=delete()>delete</span></li>
           </ul>`)
    ...
    csgServicesFactory.getAllProducts().then(function(values){
      ...
      csgServicesFactory.getAllAssets().then(function(data){  
            ...
            let productsAvailable = data.d.results.filter(function(v){
                 angular.forEach(v.related_ProductsId.results,function(value,key){
                    if(value == currentAsset.ID){
                      $scope.productsAvailable.push(v);
                    }
                 })
            });
      })
    ...
    $scope.selectAssets = function($evnt,asset){
       ...
       /*  var currentUserData = currentUser.get();
        ...
        var obj = {
          ...
          Type: aTypesArr,
          ID: asset.ID,
          selectedType: aTypesArr.length > 0 ? aTypesArr[0] : '$'
        }
        console.log('obj',obj);
  ...
  $scope.back = function(){
   ...
   /*  $scope.open = function (parentSelector) {
    ...
    $(document).on("click", ".delete" , function() {
      ...
      for(var i=0;i<dataDocumentUpload.length;i++){
        if(dataDocumentUpload[i].indexOf(name) !== -1){
          dataDocumentUpload.splice(i,1);
          dataDocumentUploadNames.splice(i,1);
        }
        
        }
        if(dataDocumentUpload.length < 1){
          $scope.docUploaded = false;
        }
      $(this).parent().remove();
  });
...
var getFormatedDate = function(){
  ...
  var ampm = date.getHours() >= 12 ? 'PM' : 'AM';
      return date.getDate() + '/' + monthNames[date.getMonth()] + '/' + date.getFullYear() +' ' + date.getHours() + ':'+ (date.getMinutes() > 9 ? date.getMinutes() : '0' + date.getMinutes()) + ' ' + ampm 
